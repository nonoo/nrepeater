...
static void open_one_device (char *devname)
{
    oss_audioinfo ai;
    int fd;
    int tmp;
    ...
    {
	fprintf (stderr, "%s doesn't support one device based full duplex scheme\n", devname);
    ...
    {
	fprintf (stderr, "Warning: Using 16 bit sample format with wrong endianess.\n");
    }
...
static void open_two_devices (char *devname_out, char *devname_in)
{
    ...
    {
	fprintf (stderr, "Note! %s and %s are not necessarily driven by the same clock.\n", devname_out, devname_in);
    }
    ...
    if (devcaps & PCM_CAP_DUPLEX)
    {
	fprintf (stderr, "Device %s supports duplex so you may want to use the single device approach instead\n", devname_out);
    }
    ...
    if (tmp != format)
    {
	fprintf (stderr, "Error: Input and output devices use different formats (%x/%x)\n", tmp, format);
	exit (-1);
    }
    if (format == AFMT_S16_OE)
    {
...
static void method_0 (int fd_out, int fd_in)
{
    int l;
    int loopcount = 0;
...
static void method_1 (int fd_out, int fd_in)
{
    char silence[32 * 1024];	/* Buffer for one fragment of silence */
    fd_set reads;
    int n;
